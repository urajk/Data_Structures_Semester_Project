...
namespace Lib.Data.Algorithms
{
    public class CAlgorithmForLinkedList<T>
    {
        ...
        protected int compare(T? p_oCurrentValue, T? p_oValue)
        {
            int nResult = 1;

            if ((p_oCurrentValue != null) && (p_oValue != null))
            { 
                // Default comparison if the class of the items supports the IComparable interface
                IComparable<T>? iThisNodeValue = p_oCurrentValue as IComparable<T>;
        ...
        protected int compare(IItemComparison<T>? p_iComparison, T? p_oCurrentValue, T? p_oValue)
        {
            int nResult = 1;
            ...
            { 
                if ((p_oCurrentValue != null) && (p_oValue != null) && (p_oCurrentValue != null))
                    nResult = p_iComparison.Compare(p_oCurrentValue, p_oValue);
            }
            else
            { 
                // Calls the other form of the method
                if ((p_oCurrentValue != null) && (p_oValue != null))
                    nResult = compare(p_oCurrentValue, p_oValue);
            }
            return nResult;
        }
        //--------------------------------------------------------------------------
